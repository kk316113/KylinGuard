...
//
// The scene classification is display-only and must not route tools or decide
// workflow order. Agent Loop next_action remains the only planning path.
func AttachScenarioWorkspaceMetadata(resp *RunResponse, task string, status string) {
	if resp == nil {
		return
	}
	if resp.RunID == "" && resp.TaskID == "" {
		resp.RunID = newTaskID()
...
// ClassifySceneTypeForDisplay is UI metadata only. It is deliberately kept out
// of planner and Agent Loop logic so it cannot become scenario workflow routing.
func ClassifySceneTypeForDisplay(task string) string {
	t := strings.ToLower(strings.TrimSpace(task))
	if t == "" {
		return "unknown"
	}
	if containsAnySceneToken(t, []string{
		"安全吗", "清空", "删除", "审计", "日志", "权限", "风险",
		"safe", "risk", "audit", "log", "delete", "clear", "permission",
	}) {
		return "security_check"
	}
	if containsAnySceneToken(t, []string{
		"卡", "慢", "cpu", "内存", "磁盘", "负载",
		"memory", "disk", "load", "slow",
	}) {
		return "system_health"
	}
	if containsAnySceneToken(t, []string{
		"服务", "端口", "访问不了", "启动", "挂了",
		"service", "port", "unreachable", "down", "start",
	}) {
		return "service_recovery"
	}
	if containsAnySceneToken(t, []string{
		"ssh", "连不上", "登录", "异常", "login", "connect",
	}) {
		return "diagnosis"
	}
	if containsAnySceneToken(t, []string{
		"合规", "基线", "整改", "compliance", "baseline",
	}) {
